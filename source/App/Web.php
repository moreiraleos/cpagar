...
class Web extends Controller
{
    ...
    public function blogCategory(array $data): void
    {
        ...
        $head = $this->seo->render(
            ...
            ($category->cover ? image($category->cover, 1200, 628) : theme("/assets/images/shares.jpg"))
        );
    ...
    {
        $postAuthor = filter_var($data['autor'], FILTER_SANITIZE_NUMBER_INT);
        $author = (new User())->findById($postAuthor);

        if (!$author) {
            redirect("/blog");
        }

        $blogAuthor = (new Post())->findPost("author = :a", "a={$author->id}");

        $page = (!empty($data["page"]) && filter_var($data["page"], FILTER_VALIDATE_INT) >= 1 ? $data["page"] : 1);
        $pager = new Pager(url("/blog/autor/{$author->id}/"));

        $pager->pager($blogAuthor->count(), 9, $page);
        $head = $this->seo->render(
            "Artigos por {$author->first_name} {$author->last_name} - " . CONF_SITE_NAME,
            "",
            url("/blog/autor/{$author->id}/{$page}"),
            ($author->photo ? image($author->photo, 1200, 628) : theme("/assets/images/shares.jpg"))
        );
        ...
        echo $this->view->render(
            ...
            [
                "head" => $head,
                "title" => "Artigos por {$author->first_name} {$author->last_name}",
                "",
                "blog" => $blogAuthor
                    ->limit($pager->limit())
                    ->offset($pager->offset())
