...
class Control extends Admin
{
    ...
    public function subscription(array $data): void
    {
        ...
        if (!$subscription) {
            ...
            redirect("/admin/control/subscription");
        }
    ...
    public function plan(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            ...
            if ($planDelete->subscribers(null)->count()) {
                $json["message"] = $this->message->warning("Não é possível remover planos com assinturas...")->render();
                echo json_encode($json);
                return;
            }
        ...
        if (!empty($data["plan_id"])) {
            ...
            $planEdit = (new AppPlan())->findById($plan_id);
        }
        ...
        echo $this->view->render("widgets/control/plan", [
            ...
            "head" => $head,
            "plan" => $planEdit,
            "subscribers" => ($planEdit ? $planEdit->subscribers()->count() : null),
        ]);
    }
}
