...
class Blog extends Admin
{
    ...
    public function post(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            // $json["redirect"] = url("/admin/blog/post/{$postEdit->id}");
        ...
        $categories = (new Category())->find("type = :type", "type=post")->order("title");
        $authors = (new User())->find("level >= :level", "level=5");
    ...
    public function category(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            if (!$categoryEdit) {
                $this->message->error("Você tentou editar uma categoria que não existe ou removida")->flash();
        ...
        // DELETE
        if (!empty($data["action"]) && $data["action"] == "delete") {
            $dataId = filter_var($data["category_id"], FILTER_SANITIZE_NUMBER_INT);

            $categoryDelete = (new Category())->findById($dataId);

            if (!$categoryDelete) {
                $json["message"] = $this->message->error("A categoria não existe ou já foi removida antes")->render();
                echo json_encode($json);
                return;
            }
