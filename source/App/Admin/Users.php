...
class Users extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$users->count()) {
                $this->message->info("Sua pesquisa não retornou resulados")->flash();
    ...
    public function user(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            //     $this->message->error("Você tentou gerenciar um usuário...")->flash();
            //     echo json_encode(["redirect" => url("/admin/users")]);
            //     return;
            // }
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            ...
            if (!$userDelete) {
                $this->message->error("Você tentou excluir um usuário que não existe...")->flash();
                echo json_encode(["redirect" => url("/admin/users/home")]);
                return;
            }
        ...
        if (!empty($data["user_id"])) {
            ...
            if (!$userEdit) {
                $json["message"] = $this->message->error("Você tentou editar um usuário que não existe ou foi excluído");
                $json["redirect"] = url("/admin/users");
                json_encode($json);
                return;
            }
