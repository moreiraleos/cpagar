...
class CafeAoi extends Controller
{
    ...
    public function __construct()
    {
        ...
        $request = $this->requestLimit("cageApi", 1, 1);
     ...
     * @return CafeAoi
     */
    protected function call(int $code, string $type = null, string $message = null, string $rule = "errors"): CafeAoi
    {
     ...
     * @return CafeAoi
     */
    protected function back(array $response = null): CafeAoi
    {
        if (!empty($response)) {
    ...
    protected function requestLimit(string $endpoint, int $limit, int $seconds, bool $attempt = false): bool
    {
        ...
        if (!$userToken) {
            $this->call(
                400,
                "invlid_data",
                "Você precisa informar seu e-mail e senha pra continuar"
            )->back();
            return false;
        }
        ...
        if ($cache[$endpoint]->requests > $limit) {
            $this->call(
                400,
                "requests_limit",
                "Você excedeu o limite de requisições para esta ação"
            )->back();
            return false;
        }
