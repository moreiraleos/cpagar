...
class Pay extends Controller
{
    ...
    {
        $user = Auth::user();
        $subscribe = (new AppSubscription())->find("user_id = :user AND status != status", "user={$user->id}&status=canceled")->fetch();

        if (!$subscribe) {

            $json["message"] = $this->message->error("Ooops! Você não tem uma assintura ativa")->render();
            echo json_encode($json);
            return;
        }
